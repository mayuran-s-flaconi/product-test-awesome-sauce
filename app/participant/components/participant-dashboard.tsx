...
export default function ParticipantDashboard() {
  ...
  const getParticipationStatus = (lottery: Lottery) => {
    ...
    const participation = lottery.participants?.find(
      (p) => p.user_id === user.id
    );
  ...
  const myParticipations = lotteries.filter((l) =>
    l.participants?.some((p) => p.user_id === user.id)
  ).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      {throwError("Somnething went wrong!")}
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {lotteries.length > 0
                  ? Math.round((myParticipations / lotteries.length) * 100)
                  : 0}
                %
              </div>
            </CardContent>
